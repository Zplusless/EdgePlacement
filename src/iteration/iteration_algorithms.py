...
class GreedyPlacer(ServerPlacer):
    ...
    def place_server(self, base_station_num, maad):
        ...
        # 将group翻译成放置结果
        edge_servers = self.group2edgeservers(self.group_list)
    ...
    def group2edgeservers(self, group_list):
        # 将group翻译成放置结果
        edge_servers = []
        for edge_id, group in enumerate(group_list):
...
class PSOPlacer(ServerPlacer):
    ...
    def place_server(self, base_station_num, maad):
        logging.info("{0}:Start running PSO with bs_num={1}, edge_coverage={2}".format(
            datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            base_station_num, maad))
        ...
        for generation in range(self.MAX_GENERATION):
            print("\n\n==============覆盖半径", self.maad, '  基站数目', base_station_num, "  第", generation,
                  "代==================")
            ...
            # ============================记录每次迭代的结果=============================
            iteration_record.append([generation, self.g_best.profit])
            # ==========================================================================
        ...
        with open(r'../result_iteration/iter_PSO_{}'.format(time_now), 'wb') as f:
            pickle.dump(iteration_record, f)
        # ==========================================================================
